        ...
        {episodes.map((ep, idx) => {
          ...
          return (
            <button
              ...
              className={`flex items-start gap-4 p-3 rounded-lg text-left transition-colors hover:bg-white/8 group ${
                isCurrent ? 'bg-white/5 border border-white/10' : ''
              }`}
            >
              {/* Episode number */}
              <span className="text-gray-500 text-lg font-bold w-6 flex-shrink-0 mt-1 group-hover:text-white transition-colors">
                {idx + 1}
              </span>

              {/* Thumbnail */}
              <div className="relative w-28 lg:w-32 h-16 flex-shrink-0 rounded overflow-hidden bg-gray-800">
                <img
                  src={ep.coverImage}
                  alt={ep.title}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className={`font-medium text-sm truncate ${isCurrent ? 'text-white' : 'text-gray-200'}`}>
                    {ep.title}
                  </p>

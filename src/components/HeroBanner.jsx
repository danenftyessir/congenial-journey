...
const HeroBanner = () => {
  ...
  const { watchProgress, recentlyWatched } = useApp();
  ...
  // Derive backdrop video: explicit field → first video file in media → null
  // YouTube embeds cannot be used as backdrop, so only type === 'video' (src-based)
  const backdropVideo =
    memory.backdropVideo ||
    memory.media?.find((m) => m.type === 'video')?.src ||
    null;

  // Reset + start the photo → video transition whenever the hero memory changes
  useEffect(() => {
    ...
    // Show photo for 1.5 s, then crossfade to video
    timerRef.current = setTimeout(() => setVideoVisible(true), 1500);
    return () => clearTimeout(timerRef.current);
  }, [memory.id, backdropVideo]);

  const toggleMute = () => {
    if (!videoRef.current) return;
    const next = !videoRef.current.muted;
    videoRef.current.muted = next;
    // Re-trigger play so the browser activates the audio context
  ...
  const handleVideoMeta = () => {
    ...
    // audioTracks is undefined in Safari — assume audio present as fallback
  ...
  return (
    <section className="relative bg-black h-[100svh] min-h-[500px] max-h-[900px] flex justify-center items-end overflow-hidden">
      ...
      {/* ── Layer 1: backdrop photo (always visible) ─────────────────────── */}
      <img
      ...
      {/* ── Layer 2: video — loads silently, crossfades in after delay ────── */}
      {backdropVideo && (
        <video
          ref={videoRef}
          src={backdropVideo}
      ...
      {/* ── Mute button — only shown if video is live AND has audio track ─── */}
      {backdropVideo && videoVisible && hasAudio && (
        <div className={`absolute right-4 sm:right-5 lg:right-10 top-20 sm:top-[18%] z-10 transition-opacity duration-500 ${videoVisible ? 'opacity-100' : 'opacity-0'}`}>
          <button
            onClick={toggleMute}
      ...
      <div className="w-full max-w-screen-2xl px-4 sm:px-5 lg:px-10 mb-[8%] sm:mb-[7%] flex flex-col gap-2 lg:gap-3 z-10 text-white">
        ...
        {/* Description — hidden on small phones */}
        <p
      ...
      {/* ── Bottom-right badges — desktop only ───────────────────────────── */}
      <div
